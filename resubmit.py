...
resubmit_jdl_filename = 'resubmit_STEP_mini.jdl'
...
parser.add_argument("-t", "--test", default=False, action="store_true",help="")
...
os.system("cp {}/queue.dat .".format(args.jobDir))
...
batchName = "resub_for_"+cluster if args.batch is None else args.batch
with open(args.jobDir+'/'+submit_filename) as f:
  submit_string_original = f.read()

  submit_string = ""
  submit_string += '\n'+submit_string_original

  last_line = submit_string.strip().rsplit('\n', 1)[1]
  submit_string = submit_string.strip().rsplit('\n', 1)[0]
...
with open('queue.dat') as f:
    for i, line in enumerate(f.readlines()):
        lhe, num, out = line.split()
        subdir_by_proc[i] = out
        num_by_proc[i] = int(num)

#print(subdir_by_proc)
#print(num_by_proc)

# move/delete old output
for proc in procs:
  base = 'miniAOD_'
  num = str(num_by_proc[proc])
  file_to_remove = 'miniAOD_{}_proc{}.root'.format(num, str(proc))
  subdir = subdir_by_proc[proc]

  if output_eos:
    #rm_command = 'eos root://cmseos.fnal.gov rm ' + output_area + '/' + subdir+ '/' + file_to_remove + ' 2> /dev/null'
    rm_command = 'eos root://cmseos.fnal.gov rm ' + subdir+ '/' + file_to_remove + ' 2> /dev/null'
  else: # local
    rm_command = 'rm -f ' + output_area + '/' + file_to_remove + ' 2> /dev/null'

  if args.verbose: print("DEBUG: remove command ", rm_command)
  try:
    if not args.test: output = subprocess.check_output(rm_command, shell=True)
  except Exception:
    pass

# remove held jobs, if any
#try:
#    if not args.test: print(subprocess.check_output("condor_rm -constraint JobStatus==5 -name {} {}".format(schedd_ad["Name"], cluster), shell=True))
#except Exception:
#    pass
...
with open(resubmit_jdl_filename, 'w') as f:
  f.write(submit_string)
proc = subprocess.Popen('condor_submit '+resubmit_jdl_filename, stdout=subprocess.PIPE, shell=True)
(out, err) = proc.communicate()
out = ((out.decode('utf-8')).split('\n'))[1]
cluster_id = (out.split()[-1])[:-1]
print(cluster_id)
...
os.system("rm queue.dat")
os.system("rm "+ resubmit_jdl_filename)
